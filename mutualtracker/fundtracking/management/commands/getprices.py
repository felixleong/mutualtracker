...
class Command(NoArgsCommand):
    PB_PRICES_URL = (
        'http://www.publicmutual.com.my/application/fund/'
        'fundprice.aspx')
    EXPECTED_HEADER = [
        'Date',
        'Fund',
    ...
    def handle_noargs(self, **options):
        ...
        if page.getcode() != 200:
            raise CommandError(
                'Fail to fetch the page - status {0}'.format(page.getcode()))

        # Correctly identify the rows
        soup = BeautifulSoup(page)
        fund_table = soup.find('table', 'fundtable')
        if fund_table.tbody is not None:
        ...
        header = [
            col.find(text=True) for col in fund_rows[0].findAll('td')]
        ...
        for row in fund_rows[1:]:
            ...
            update_date = datetime.strptime(
                data[0].find(text=True).string, '%d/%m/%Y')
